...
export default function Workspace({ user }: {
    ...
    const [rename, setRename] = useState<{kind:'folder'|'package';id:string;previousName:string;revision:number} | null>(null);
    ...
    function startRename(kind: 'folder'|'package', id: string, name: string) {
    ...
    async function saveRename(e: React.FormEvent) {
        ...
        try {
            const response = await fetch(rename.kind === 'folder' ? '/api/subsystems' : '/api/packages', {
                method:'POST', headers:{'Content-Type':'application/json'},
                body:JSON.stringify({action:'rename',id:rename.id,name:renameName,previousName:rename.previousName,revision:rename.revision})
            });
            const result = await response.json() as {error?:string};
            if (!response.ok) throw new Error(result.error || 'Could not save the name.');
            setRename(null); await refresh(); toast.success(rename.kind === 'folder' ? 'Folder renamed' : 'Package renamed');
    ...
    useEffect(() => { const context = (document as any).modelContext; if (!context?.registerTool)
        ...
        Promise.resolve(context.registerTool({ name: 'view_aero_subsystem', title: 'View subsystem', description: 'Select a subsystem in the CAD workspace. Does not change files.', inputSchema: { type: 'object', properties: { subsystem: { type: 'string', enum: ['All subsystems', ...SUBSYSTEMS.map(folderLabel)] } }, required: ['subsystem'], additionalProperties: false }, annotations: { readOnlyHint: true }, execute: async (input: any) => { const id = input?.subsystem === 'All subsystems' ? 'All subsystems' : SUBSYSTEMS.find(id => folderLabel(id) === input?.subsystem); if (!id)
 ...
 <Dialog open={!!rename} onOpenChange={open => { if (!open && !renaming) setRename(null); }}><DialogContent showCloseButton={!renaming}><DialogHeader><DialogTitle>{rename?.kind === 'folder' ? 'Rename subsystem folder' : 'Rename design package'}</DialogTitle><DialogDescription>{rename?.kind === 'folder' ? 'This name is shared across your team. Existing packages stay in this folder.' : 'Change the name shown in Aero Vault. SolidWorks filenames, assembly references, and revision history stay the same.'}</DialogDescription></DialogHeader><form onSubmit={saveRename} className="space-y-4">
 {rename?.kind === 'folder' && <label className="field">Folder<Choice value={rename.id} options={SUBSYSTEMS} labels={labels} disabled={renaming} onChange={id => { setRename({...rename,id}); setRenameName(folderLabel(id)); setRenameError(''); }}/></label>}
 <label className="field">New name<input autoFocus required maxLength={rename?.kind === 'folder' ? 60 : 100} value={renameName} disabled={renaming} onChange={e => setRenameName(e.target.value)}/></label>
 {renameError && <p role="alert" className="text-sm text-red-700">{renameError}</p>}
 <div className="flex justify-end gap-2"><Button type="button" variant="outline" disabled={renaming} onClick={() => setRename(null)}>Cancel</Button><Button type="submit" disabled={renaming || !renameName.trim()}>{renaming ? 'Saving…' : 'Save name'}</Button></div>
