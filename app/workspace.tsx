...
export default function Workspace({ user }: {
 ...
 {isAdmin && view === 'Design files' && <div className="flex flex-wrap gap-2 mb-5"><Button variant="outline" onClick={() => startRename('new-folder', '', '')}><Plus size={16}/>Add subsystem folder</Button><Button variant="outline" onClick={() => { const id = subsystemIds.includes(folder) ? folder : 'Wings'; startRename('folder', id, folderLabel(id)); }}><Pencil size={16}/>Rename folders</Button></div>}
 ...
 {rename?.kind === 'folder' && <label className="field">Folder<Choice value={rename.id} options={subsystemIds} labels={labels} disabled={renaming} onChange={id => { setRename({...rename,id}); setRenameName(folderLabel(id)); setRenameError(''); }}/></label>}
