...
import { Plane, Folder, Upload, Lock, Unlock, History, Users, LayoutGrid, ChevronRight, ArrowUpRight, FileBox, Download, RefreshCw, Check, Plus, ShieldCheck, Activity, Loader2, Info } from 'lucide-react';
...
const SUBSYSTEMS = ['Wings', 'Fuselage', 'Empennage', 'Propulsion', 'Landing gear'];
type Package = {
    id: string;
    name: string;
    subsystem: string;
...
function Choice({ value, onChange, options, disabled = false }: {
    value: string;
    onChange: (v: string) => void;
    options: string[];
    disabled?: boolean;
}) { return <Select value={value} onValueChange={onChange} disabled={disabled}><SelectTrigger className="w-full h-11 bg-white"><SelectValue /></SelectTrigger><SelectContent>{options.map(x => <SelectItem key={x} value={x}>{x}</SelectItem>)}</SelectContent></Select>; }
export default function Workspace({ user }: {
    ...
    const filtered = packages.filter(p => (folder === 'All subsystems' || p.subsystem === folder) && (view !== 'Checked out' || !!p.locked_by));
    const startUpload = (p: Package | 'new') => { setUpload(p); setName(p === 'new' ? '' : p.name); setSubsystem(p === 'new' ? (SUBSYSTEMS.includes(folder) ? folder : 'Wings') : p.subsystem); setNote(''); setFile(null); setProgress(0); setUploadError(''); };
    useEffect(() => { const context = (document as any).modelContext; if (!context?.registerTool)
        ...
        Promise.resolve(context.registerTool({ name: 'view_aero_subsystem', title: 'View subsystem', description: 'Select a subsystem in the CAD workspace. Does not change files.', inputSchema: { type: 'object', properties: { subsystem: { type: 'string', enum: ['All subsystems', ...SUBSYSTEMS] } }, required: ['subsystem'], additionalProperties: false }, annotations: { readOnlyHint: true }, execute: async (input: any) => { if (!input || !['All subsystems', ...SUBSYSTEMS].includes(input.subsystem))
                throw new Error('Unknown subsystem'); setFolder(input.subsystem); setView('Design files'); return { subsystem: input.subsystem }; } }, { signal: lifecycle.signal })).catch(() => { });
    }
    ...
    async function sendUpload(e: React.FormEvent) { e.preventDefault(); if (!file || !upload)
        ...
        await new Promise<void>((resolve, reject) => { const xhr = new XMLHttpRequest(); xhr.open('POST', '/api/packages?' + query); xhr.setRequestHeader('Content-Type', 'application/octet-stream'); xhr.timeout = 300000; xhr.upload.onprogress = e => { if (e.lengthComputable)
    ...
    return <SidebarProvider className={native.available ? "native-workspace" : ""}><Toaster richColors position="bottom-right"/><Sidebar className="border-0"><SidebarHeader className="px-6 pt-8 pb-7"><div className="flex items-center gap-3"><div className="size-10 rounded-xl bg-blue-500 flex items-center justify-center"><Plane size={23}/></div><div><span className="text-xl tracking-tight font-bold">Aero Vault</span><p className="text-xs text-slate-400 mt-1">SAE AERO · TEAM WORKSPACE</p></div></div></SidebarHeader><SidebarContent><SidebarGroup className="px-4"><SidebarGroupLabel>WORKSPACE</SidebarGroupLabel><SidebarMenu>{[['Design files', LayoutGrid], ['Checked out', Lock], ['Activity', History], ['Team', Users]].map(([label, Icon]) => <SidebarMenuItem key={String(label)}><SidebarMenuButton className="h-11 px-3 text-sm" isActive={view === label} onClick={() => { setView(String(label)); setFolder('All subsystems'); }}><Icon /><span>{String(label)}</span>{label === 'Checked out' && packages.some(p => p.locked_by) && <span className="ml-auto text-xs">{packages.filter(p => p.locked_by).length}</span>}</SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu></SidebarGroup><SidebarGroup className="px-4 mt-5"><SidebarGroupLabel>SUBSYSTEMS</SidebarGroupLabel><SidebarMenu>{SUBSYSTEMS.map(s => <SidebarMenuItem key={s}><SidebarMenuButton className="h-10 px-3 text-slate-300" isActive={folder === s} onClick={() => { setFolder(s); setView('Design files'); }}><Folder /><span>{s}</span></SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu></SidebarGroup></SidebarContent><SidebarFooter className="p-5 gap-5"><div className="rounded-lg border border-slate-600/50 p-3 text-sm text-slate-300"><div className="flex items-center gap-2 text-white mb-1"><ShieldCheck size={16}/>Team access</div><p className="text-xs leading-5">{members.length} of 9 places assigned</p></div><div className="flex gap-3 items-center"><div className="size-9 shrink-0 rounded-full bg-slate-600 flex items-center justify-center text-xs font-bold">{user.name.slice(0, 2).toUpperCase()}</div><div className="min-w-0"><div className="text-sm truncate">{user.name}</div><div className="text-xs text-slate-400 capitalize">{data?.member?.role || 'Workspace owner'}</div></div></div></SidebarFooter></Sidebar>
 ...
 <div className="p-5 lg:p-9 max-w-[1600px] w-full mx-auto">{native.available && <nav aria-label="Workspace navigation" className="grid gap-3 mb-5"><label className="field">Workspace view<Choice value={view} onChange={v => { setView(v); setFolder('All subsystems'); }} options={['Design files', 'Checked out', 'Activity', 'Team']}/></label>{view === 'Design files' && <label className="field">Subsystem<Choice value={folder} onChange={setFolder} options={['All subsystems', ...SUBSYSTEMS]}/></label>}</nav>}<div className="flex flex-wrap justify-between gap-4 items-start mb-7"><div><p className="kicker mb-2">AIRCRAFT DEVELOPMENT</p><h1 className="text-3xl font-semibold tracking-tight">{folder === 'All subsystems' ? view : folder}</h1><p className="quiet text-sm mt-2">{view === 'Team' ? 'Manage the people who can access your designs.' : view === 'Activity' ? 'A shared record of your team’s design changes.' : view === 'Checked out' ? 'See who is working on each design package.' : 'One place for your team’s latest designs.'}</p></div>{view === 'Team' ? <Button className="h-11 px-5" disabled={!isAdmin || members.length >= 9} onClick={() => setInvite(true)}><Plus size={17}/>Add teammate</Button> : view !== 'Activity' && <Button className="h-11 px-5" disabled={!canEdit} onClick={() => startUpload('new')}><Upload size={17}/>Upload package</Button>}</div>
 ...
 <footer className="mt-8 text-xs quiet flex flex-wrap justify-between gap-3"><span>AERO VAULT <span className="mx-2 text-slate-300">/</span> SAE Aero</span><span>{native.available ? "SolidWorks panel · Revision history · Team checkout" : "Manual uploads · Revision history · Team checkout"}</span></footer></div></SidebarInset>
 <Dialog open={!!upload} onOpenChange={o => { if (!o && !uploading && !native.preparing)
        setUpload(null); }}><DialogContent showCloseButton={!uploading && !native.preparing}><DialogHeader><DialogTitle>{upload === 'new' ? 'Upload a design package' : 'Check in a new revision'}</DialogTitle><DialogDescription>{upload === 'new' ? 'Keep linked parts together with a Pack and Go ZIP.' : 'Your previous revisions stay available. Saving releases your checkout.'}</DialogDescription></DialogHeader><form onSubmit={sendUpload} className="space-y-4"><label className="field">Package name<input required maxLength={100} value={name} disabled={upload !== 'new' || uploading} onChange={e => setName(e.target.value)} placeholder="e.g. Main wing assembly"/></label><label className="field">Subsystem<Choice value={subsystem} onChange={setSubsystem} options={SUBSYSTEMS} disabled={upload !== 'new' || uploading}/></label>{native.available && <div className="rounded-lg border border-blue-200 bg-blue-50 p-4"><Button type="button" variant="outline" className="w-full" disabled={uploading || native.preparing} onClick={async () => { setUploadError(''); try { const prepared = await native.packageActive(); setFile(prepared); if (!name.trim()) setName(prepared.name.replace(/\.zip$/i, '')); } catch (e) { setUploadError(e instanceof Error ? e.message : 'Could not prepare the active design.'); } }}>{native.preparing ? <Loader2 size={16} className="animate-spin"/> : <FileBox size={16}/>} {native.preparing ? 'Preparing active design…' : 'Use active SolidWorks design'}</Button><p className="text-xs quiet mt-2">Save your open designs first. The add-in collects a Pack and Go ZIP; you review it before uploading.</p></div>}<label className="field">Design file<input type="file" required={!file} accept=".zip,.sldprt,.sldasm,.slddrw" disabled={uploading || native.preparing} onChange={e => setFile(e.target.files?.[0] || null)} className="text-sm"/><span className="text-sm font-normal">{file ? `Selected: ${file.name} · ${bytes(file.size)}` : native.available ? "Choose a file or prepare the active design." : "Choose your design file."}</span><span className="text-xs quiet font-normal">Up to 50 MB. Individual assembly files do not include their linked parts.</span></label><label className="field">Change notes<textarea required maxLength={2000} value={note} disabled={uploading} onChange={e => setNote(e.target.value)} rows={3} placeholder="What is included or what changed?"/></label>{uploadError && <p role="alert" className="text-sm text-red-700">{uploadError}</p>}{uploading && <div aria-live="polite"><Progress value={progress}/><p className="text-xs quiet mt-2">{progress === 100 ? 'Saving your revision…' : `Uploading · ${progress}%`}</p></div>}<Button type="submit" className="w-full h-11" disabled={uploading || native.preparing || !file}>{uploading ? <Loader2 className="animate-spin" size={16}/> : <Upload size={16}/>} {uploading ? 'Saving…' : upload === 'new' ? 'Upload package' : 'Save revision & check in'}</Button></form></DialogContent></Dialog>
 <Sheet open={!!current} onOpenChange={o => { if (!o)
        setSelected(null); }}><SheetContent className="sm:max-w-xl overflow-y-auto p-0">{current && <><SheetHeader className="p-6 border-b"><p className="kicker mb-2">{current.subsystem}</p><SheetTitle className="text-2xl">{current.name}</SheetTitle><SheetDescription>Revision {current.version} · Updated {date(current.updated_at)}</SheetDescription></SheetHeader><div className="p-6 space-y-7"><div className="rounded-lg bg-slate-50 border p-4"><p className="text-sm font-semibold flex items-center gap-2">{current.locked_by ? <Lock size={16}/> : <Unlock size={16}/>} {current.locked_by ? `Checked out by ${current.locked_by === user.id ? 'you' : current.locked_name}` : 'Available for checkout'}</p><p className="text-sm quiet mt-2">{current.locked_by ? 'Other teammates can download, but only the checkout holder can upload a revision.' : 'Reserve this package before editing to prevent conflicting uploads.'}</p></div><div className="flex flex-wrap gap-2">{native.available && <Button disabled={busy || native.preparing} onClick={() => native.openRevision(current.current_version_id, current.locked_by !== user.id)}>Open in SolidWorks{current.locked_by !== user.id ? " (read-only)" : ""}</Button>}<Button asChild variant="outline"><a href={'/api/download?id=' + current.current_version_id}><Download size={16}/>Download latest</a></Button>{canEdit && !current.locked_by && <Button disabled={busy} onClick={() => void act('/api/packages', { id: current.id, action: 'checkout' }, 'Package checked out')}><Lock size={16}/>Check out</Button>}{current.locked_by === user.id && <Button disabled={busy} onClick={() => startUpload(current)}><Upload size={16}/>Check in revision</Button>}{canEdit && current.locked_by && (current.locked_by === user.id || isAdmin) && <Button variant="ghost" disabled={busy} onClick={() => void act('/api/packages', { id: current.id, action: 'release' }, 'Checkout released')}>Release checkout</Button>}</div><div className="field">Design status<Choice value={current.status} onChange={status => void act('/api/packages', { id: current.id, action: 'status', status, base: current.version }, 'Status updated')} options={isAdmin ? ['In progress', 'Ready for review', 'Approved'] : current.status === 'Approved' ? ['Approved'] : ['In progress', 'Ready for review']} disabled={!canEdit || !!current.locked_by || busy || (!isAdmin && current.status === 'Approved')}/></div><section><h3 className="font-semibold mb-4 flex gap-2 items-center"><History size={17}/>Revision history</h3><div className="space-y-3">{versions.filter(v => v.package_id === current.id).map(v => <div key={v.id} className="rounded-lg border p-4"><div className="flex justify-between gap-3"><span className="text-sm font-semibold">Revision {v.revision}{v.revision === current.version && <span className="ml-2 text-xs text-blue-600">Latest</span>}</span><a className="text-blue-700 text-sm flex items-center gap-1" href={'/api/download?id=' + v.id}><Download size={14}/>Download</a></div><p className="text-sm leading-6 mt-3 whitespace-pre-wrap break-words">{v.note}</p><p className="text-xs quiet mt-3 break-all">{v.filename} · {bytes(v.size)}</p><p className="text-xs quiet mt-1">{v.author_name} · {date(v.created_at)}</p></div>)}</div></section></div></>}</SheetContent></Sheet>
