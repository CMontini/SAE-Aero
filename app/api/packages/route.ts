    ...
    try {
        ...
        if (url.searchParams.get('action') === 'upload') {
            ...
            const completed = async () => {
                ...
                if (!v) return null;
                if (v.package_id !== id || v.author_id !== m.user_id || v.revision !== base + 1 || v.filename !== filename || v.size !== length || v.note !== note)
            ...
            if (p) {
                ...
                const result = await db.batch([
                    db.prepare(`INSERT OR IGNORE INTO versions (id,package_id,revision,filename,size,storage_key,note,author_id,author_name,created_at) SELECT ?,?,?,?,?,?,?,?,?,? WHERE EXISTS (SELECT 1 FROM packages WHERE id=? AND version=? AND ${guard})`).bind(versionId, id, rev, filename, length, key, note, m.user_id, m.name, fresh, id, base, ...args),
                    db.prepare("UPDATE packages SET version=?,current_version_id=?,locked_by=?,lock_token=?,locked_at=?,updated_at=?,status='In progress' WHERE id=? AND version=? AND EXISTS (SELECT 1 FROM versions WHERE id=? AND storage_key=?)").bind(rev, versionId, session ? m.user_id : null, session || null, session ? fresh : null, fresh, id, base, versionId, key)
                ]);
